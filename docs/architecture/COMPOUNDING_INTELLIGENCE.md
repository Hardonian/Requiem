...
- Added middleware proxy + route handler integration harness test for intelligence APIs.
- Added Zod schema contracts for `economic_events.ndjson` and `artifacts.ndjson` extraction inputs.
- Cases UI now surfaces extracted `cost_units` and artifact pointers.

- Added dedicated extraction regression fixtures (`ready-layer/tests/fixtures/intelligence/extraction`) for synthetic economic/artifact enrichment inputs.
- Cases UI now surfaces pointer type badges (`run`, `artifact`, `evidence`).
