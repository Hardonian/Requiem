...
> that _looks_ real but is **not** fully enforced. The goal is honest accounting — not blame.
>
...
| **Risk** | **High** — A consumer that checks `signature != ""` will believe bundles are unsigned (correct), but a consumer that assumes _any_ bundle from this system is signed will be deceived. Audit chains relying on bundle integrity have no cryptographic guarantee. |
...
| **Risk** | **High** — Tool-level audit records are not available after restart or across instances. Compliance requirements (designed to support SOC 2 readiness, GDPR, etc.) that require audit trails cannot be met with the current sink. |
...
_This document is maintained as part of the Phase 5 "No Theatre Pass". Update this file whenever a stub is promoted to a real implementation or a new stub is introduced._
