...
async function verifyRoutes() {
  ...
  // 2. Load manifest (optional — generated by scripts/generate_routes_manifest.sh)
  log('\nLoading route manifest...', 'info');
  const manifest = loadManifest();
  if (!manifest) {
    log('  ⚠ routes.manifest.json not found — skipping manifest alignment check', 'warning');
    log('  Run scripts/generate_routes_manifest.sh to generate it.', 'warning');
  } else {
  ...
  // 5. Verify manifest alignment (skipped if manifest not present)
  ...
  if (manifest) {
    ...
    log(`  API routes in filesystem: ${apiRoutes.length}`, 'info');
    log(`  Manifest not available — skipping alignment check`, 'warning');
  }
