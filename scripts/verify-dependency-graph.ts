 ...
 *
 * Usage: npx tsx scripts/verify-dependency-graph.ts
 *        node scripts/dist/verify-dependency-graph.js
 */
...
// Combined regex for import/export from, dynamic imports, and require
const IMPORT_REGEX =
  /(?:^|[;\n])\s*(?:import\s+(?:type\s+)?(?:\*\s+as\s+\w+|\{[^}]*\}|\w+(?:\s*,\s*(?:\{[^}]*\}|\*\s+as\s+\w+))?)?|export\s+(?:type\s+)?(?:\{[^}]*\}|\w+(?:\s*,\s*(?:\{[^}]*\}|\w+))?)?)\s+from\s+['"]([^'"]+)['"]|import\s*\(\s*['"]([^'"]+)['"]\s*\)|require\s*\(\s*['"]([^'"]+)['"]\s*\)/gm;
...
function parseImports(filePath: string): ImportInfo[] {
  ...
  while ((match = IMPORT_REGEX.exec(content)) !== null) {
    ...
    if (match[2]) {
      ...
      importType = 'require';
    } else if (rawImport.includes('import type') || rawImport.includes('export type')) {
    ...
    imports.push({
      ...
      line,
      rawImport: rawImport.slice(0, 100),
    });
  }
