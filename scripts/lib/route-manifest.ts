...
const PUBLIC_ROUTES = new Set(['/api/health', '/api/openapi.json', '/api/status', '/api/mcp/health']);
...
function methodsFromSource(source: string): HttpMethod[] {
  return METHODS.filter((m) => new RegExp(`export\\s+async\\s+function\\s+${m}\\s*\\(`).test(source));
}
...
export function generateRouteManifest(repoRoot = process.cwd()): RouteManifest {
  ...
  const files = walk(apiRoot).filter((file) => file.endsWith('/route.ts') || file.endsWith('\\route.ts'));
  ...
  for (const file of files) {
    ...
    for (const method of methods) {
      routes.push({
        ...
        ...(routePath === '/api/health' || routePath === '/api/mcp/health' ? { probe: true } : {}),
