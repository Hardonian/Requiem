...
We define the **Determinism Coefficient (D)** as the probability that a run $R$ replayed in environment $E_1$ matches the fingerprint generated in $E_0$. Through formal verification and engine hardening, Requiem achieves $D=1.00$ for all strictly-defined tool interfaces.
