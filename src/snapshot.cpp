...
Snapshot snapshot_create(const std::string& tenant_id) {
  ...
  snapshot.cas_root_hash = hash_domain("cas:", "snapshot_" + std::to_string(snapshot.logical_time));
...
SnapshotRestoreResult snapshot_restore(const std::string& snapshot_hash) {
  ...
  result.message = "Successfully restored from snapshot " + snapshot_hash + 
                   ". Logical time set to " + std::to_string(snapshot.logical_time);
...
Snapshot snapshot_from_json(const std::string& json) {
  ...
  snapshot.snapshot_version = static_cast<uint32_t>(jsonlite::get_u64(obj, "snapshot_version", 1));
...
std::string snapshot_compute_hash(const Snapshot& snapshot) {
  ...
  return hash_domain("snap:", jsonlite::to_json(jsonlite::Value{std::move(obj)}));
}
