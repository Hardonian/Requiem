...
BenchmarkResult run_benchmark(const std::string &name, BenchmarkFn fn,
...
std::optional<Baseline> load_baseline(const std::string &path) {
  ...
  baseline.throughput_ops_sec =
      static_cast<double>(jsonlite::get_u64(obj, "throughput_ops_sec", 0));
...
bool check_regression(const BenchmarkResult &result, const Baseline &baseline,
                      double threshold) {

  // Check p50 regression
  if (baseline.p50_ns > 0) {
    double p50_change =
        (result.latency.p50_ns - baseline.p50_ns) / baseline.p50_ns;
  ...
  if (baseline.p95_ns > 0) {
    double p95_change =
        (result.latency.p95_ns - baseline.p95_ns) / baseline.p95_ns;
  ...
  if (baseline.throughput_ops_sec > 0) {
    double throughput_change =
        (result.throughput_ops_sec - baseline.throughput_ops_sec) /
        baseline.throughput_ops_sec;
...
BenchmarkResult bench_event_append(const std::string &event_log_path,
                                   uint64_t iterations) {
...
BenchmarkResult bench_cas_put(const std::string &data, uint64_t iterations) {

  // Create CAS store in memory for benchmark
  auto backend = std::make_shared<CasStore>(".requiem/bench_cas");

  auto fn = [&backend, &data]() { backend->put(data); };

  return run_benchmark("cas_put", fn, iterations, 100);
}

BenchmarkResult bench_cas_get(const std::string &digest, uint64_t iterations) {
  ...
  auto fn = [&backend, &actual_digest]() { backend->get(actual_digest); };
...
BenchmarkResult bench_policy_eval(const std::string &policy_json,
...
BenchmarkResult bench_plan_scheduling(const std::string &plan_json,
                                      uint64_t iterations) {
...
std::string
benchmark_results_to_json(const std::vector<BenchmarkResult> &results) {
  jsonlite::Array arr;
