...
std::string to_string(DriftCategory c) {
  switch (c) {
  case DriftCategory::none:
    return "none";
  case DriftCategory::result_digest:
    return "result_digest";
  case DriftCategory::stdout_digest:
    return "stdout_digest";
  case DriftCategory::stderr_digest:
    return "stderr_digest";
  case DriftCategory::request_digest:
    return "request_digest";
  case DriftCategory::trace_digest:
    return "trace_digest";
  case DriftCategory::policy_applied:
    return "policy_applied";
  case DriftCategory::env_time_leak:
    return "env_time_leak";
  }
  return "unknown";
}
...
DriftCategory classify_drift(const requiem::ExecutionResult &primary,
  ...
  if (primary.request_digest != shadow.request_digest)
    return DriftCategory::request_digest;
  if (primary.result_digest != shadow.result_digest)
    return DriftCategory::result_digest;
  if (primary.stdout_digest != shadow.stdout_digest)
    return DriftCategory::stdout_digest;
  if (primary.stderr_digest != shadow.stderr_digest)
    return DriftCategory::stderr_digest;
  if (primary.trace_digest != shadow.trace_digest)
    return DriftCategory::trace_digest;
  // Check for potential time leak: stdout contains a digit pattern that
  // changes. Heuristic: if stdout_digest changes but command is deterministic,
  // suspect time leak.
  const auto &pa = primary.policy_applied;
  const auto &sa = shadow.policy_applied;
  if (pa.mode != sa.mode || pa.time_mode != sa.time_mode)
    return DriftCategory::policy_applied;
...
ShadowRunResult run_shadow_pair(int run_index, const std::string &tid,
  ...
  if (r.drifted) {
    r.record = DriftRecord{run_index,
                           tid,
                           req.request_id,
                           primary.request_digest,
                           primary.result_digest,
                           shadow.result_digest,
                           cat};
  }
  return r;
}
...
int shadow_main(int /*argc*/, char ** /*argv*/) {
  ...
  const double wall_s =
      std::chrono::duration<double>(Clock::now() - wall_t0).count();
  ...
  for (std::size_t i = 0; i < drifts.size(); ++i) {
    ...
    diff << "{"
         << "\"run_index\":" << d.run_index << ",\"tenant_id\":\""
         << d.tenant_id << "\""
  ...
  report << "{"
         ...
         << ",\"pass\":"
         << (promotion_gate_pass && shadow_meter_pass ? "true" : "false")
         << ",\"tenants\":" << kNumTenants
         ...
         << ",\"wall_time_s\":" << wall_s << ",\"drift_diffs_dir\":\""
         << diffs_dir << "\""
         << ",\"hash_primitive\":\"blake3\""
         << "}";

  write_file("artifacts/reports/CLOUD_DETERMINISM_REPORT.json", report.str());
  std::cout << "[shadow] report written: "
               "artifacts/reports/CLOUD_DETERMINISM_REPORT.json\n";
  std::cout << "[shadow] promotion_gate="
            << (promotion_gate_pass ? "PASS" : "FAIL")
