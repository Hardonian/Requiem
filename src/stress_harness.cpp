...
ExecMetrics run_one(const RunConfig &cfg) {
  ...
  case 1: // medium — process a fixed string
    req.command = "/bin/sh";
    req.argv = {"-c", "printf '%0.s-' {1..500} | wc -c"}; // deterministic
    break;
  case 2: // large — bounded
    req.command = "/bin/sh";
    req.argv = {"-c", "dd if=/dev/zero bs=4096 count=2 2>/dev/null | wc -c"};
    break;
  }
...
// Fail if a subsequent run for the same canonical input produces a different
// result_digest.
  ...
  bool check(const std::string &req_digest, const std::string &result_digest) {
    ...
    if (it->second != result_digest) {
      ...
      std::cerr << "DRIFT DETECTED: req=" << req_digest
                << " expected=" << it->second << " got=" << result_digest
                << "\n";
      return false;
    }
    return true;
...
struct Stats {
  ...
  std::string to_json(const std::string &phase, double wall_s) const {
    ...
    oss << "{"
        ...
        << ",\"failure\":" << failure << ",\"throughput_ops_sec\":"
        << fmt_double(total / (wall_s > 0 ? wall_s : 1.0))
        << ",\"latency_ms\":{"
        ...
        << ",\"min\":"
        << fmt_double(sorted_lat.empty() ? 0.0 : sorted_lat.front())
        << ",\"max\":"
        << fmt_double(sorted_lat.empty() ? 0.0 : sorted_lat.back()) << "}"
        << ",\"error_dist\":{";
    bool first = true;
    ...
    oss << "}";
    oss << ",\"five_xx_rate\":"
        << fmt_double(total > 0 ? (double)failure / total : 0.0) << "}";
    return oss.str();
  }
};
...
int stress_main(int /*argc*/, char ** /*argv*/) {
  ...
  for (int i = 0; i < kSeqExecutions; ++i) {
    ...
    // Drift check: only for deterministic policy runs (nonce=0, same command
    // per variant).
    if (cfg.policy_variant == 0 && !m.request_digest.empty() &&
        !m.result_digest.empty()) {
    ...
    auto ev = requiem::make_meter_event(
        cfg.tenant_id, make_request_id(cfg.tenant_id, i), m.request_digest,
        m.ok, m.error_code, /*is_shadow=*/false);
    meter.emit(ev);
  ...
  const double seq_wall =
      std::chrono::duration<double>(Clock::now() - seq_t0).count();
  ...
  const double conc_wall =
      std::chrono::duration<double>(Clock::now() - conc_t0).count();
  ...
  const bool overall_pass =
      (drift.drift_count == 0) && cas_isolation_pass && billing_error.empty();
  ...
  report << "{"
         ...
         << ",\"shadow_events\":" << meter.count_shadow() << ",\"error\":\""
         << billing_error << "\""
