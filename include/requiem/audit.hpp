...
//   2. SEQUENTIAL: each entry carries a monotonically increasing sequence
//   number.
...
//   5. PROVENANCE: every entry records engine_version + hash_version +
//   replay_flag.
//
...
//        digest to form a tamper-evident chain (like a certificate transparency
//        log).
...
//     c) Distributed replay comparison: store replay results from multiple
//     nodes
...
//
// EXTENSION_POINT: governance_enhancements
//   Add: signature field (Ed25519 over result_digest + engine_semver +
//   hash_version). Add: previous_entry_digest for Merkle-chaining.
struct ProvenanceRecord {
  uint64_t sequence{0}; // Monotonic sequence number (global counter)
  std::string
      previous_digest;      // BLAKE3 of the previous audit entry (Merkle chain)
...
// ---------------------------------------------------------------------------
// Activated by setting REQUIEM_AUDIT_LOG=/path/to/audit.ndjson before engine
// init. In Enterprise mode: configured programmatically via
// set_audit_log_path().
//
// EXTENSION_POINT: governance_enhancements
