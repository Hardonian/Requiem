...
//   1. CAS key = BLAKE3(original_bytes) ALWAYS — content-addressed, not location-addressed.
//   2. Writes are atomic: tmp+rename on the same filesystem (POSIX rename guarantee).
//   3. Reads verify integrity: stored_blob_hash is checked before returning data.
//   4. Fail-closed: any integrity failure returns empty/nullopt, never corrupted data.
//   5. Deduplication: a second put() of the same content returns the same digest immediately.
//
// EXTENSION_POINT: multi-region_cas_replication
//   Current: single-machine local filesystem (LocalFSBackend).
//   Upgrade path: implement S3CompatibleBackend or GCSBackend that maps the same
//   object_path() hierarchy to cloud storage prefixes.
//   For replication: use a ReplicatingBackend that wraps two ICASBackend instances
//   and writes to both (with rollback on secondary failure).
//   Invariant: the CAS key scheme must never change (BLAKE3, "cas:" domain prefix).
//   Changing it would invalidate all existing stored objects without migration.
...
class ICASBackend {
 public:
  ...
  virtual std::optional<CasObjectInfo> info(const std::string& digest) const = 0;
...
//   - Use conditional PutObject (x-amz-copy-source-if-none-match) for atomicity.
//   - Use multipart upload for objects > 5MB (ZSTD-compressed blobs).
//   - Metadata stored as S3 object tags or a separate .meta object.
//   - Integrity: verify ETag (MD5) on small objects; use SHA256 for larger ones.
...
//
// To activate: set REQUIEM_CAS_BACKEND=s3 and configure endpoint/bucket/credentials.
class S3CompatibleBackend : public ICASBackend {
 public:
  // endpoint: e.g. "https://s3.amazonaws.com" or "http://localhost:9000" (MinIO)
  // bucket: S3 bucket name
  // prefix: object key prefix (e.g. "cas/v2/")
