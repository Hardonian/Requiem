...
struct Value {
  std::variant<std::nullptr_t, bool, std::string, std::uint64_t, double,
...
std::string canonicalize_json(const std::string &text,
                              std::optional<JsonError> *error = nullptr);
std::string hash_json_canonical(const std::string &text,
                                std::optional<JsonError> *error = nullptr);
Object parse(const std::string &text,
             std::optional<JsonError> *error = nullptr);

// Serialize a Value tree to canonical JSON (sorted keys, no whitespace).
...
unsigned long long get_u64(const std::string &s, const std::string &key,
                           unsigned long long def = 0);
double get_double(const std::string &s, const std::string &key,
                  double def = 0.0);
