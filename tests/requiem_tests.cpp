...
void test_blake3_known_vectors() {
  ...
  expect(requiem::blake3_hex("") ==
             "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262",
         "BLAKE3 empty vector");
  // "hello" — official test vector
  expect(requiem::blake3_hex("hello") ==
...
void test_canonical_request_digest() {
  ...
  const std::string digest2 =
      requiem::deterministic_digest(requiem::canonicalize_request(req));
...
void test_canonical_result_digest() {
  ...
  const std::string digest2 =
      requiem::deterministic_digest(requiem::canonicalize_result(res));
...
void test_domain_separation() {
  ...
  expect(req_hash == requiem::hash_domain("req:", data),
         "domain hash must be deterministic");
}
...
void test_file_hashing() {
  ...
  expect(requiem::hash_file_blake3("/nonexistent").empty(),
...
void test_no_float_in_digest_path() {
  ...
  expect(canon.find("\"nonce\":12345") != std::string::npos,
         "nonce must be integer in canonical form");
  expect(canon.find("12345.") == std::string::npos,
         "nonce must not be float in canonical form");
}
...
void test_request_id_sanitization() {
  ...
  auto req = requiem::parse_request_json(
      "{\"request_id\":\"../../../etc/passwd\",\"command\":\"/bin/true\"}",
      &err);
  // Slashes must be stripped
  expect(req.request_id.find('/') == std::string::npos,
         "request_id must not contain /");
  expect(req.request_id.find("..") == std::string::npos,
         "request_id must not contain ..");
}
...
void test_cas_corruption_detection() {
  ...
  std::string obj_path = (fs::path(tmp) / "objects" / digest.substr(0, 2) /
                          digest.substr(2, 2) / digest)
                             .string();
  {
    std::fstream file(obj_path,
...
void test_cas_invalid_digest_rejected() {
  ...
  expect(!cas.get("abc").has_value(),
         "get with invalid digest returns nullopt");
  expect(!cas.info("abc").has_value(),
         "info with invalid digest returns nullopt");
...
void test_cas_compact() {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      if (i == 0) {
        ...
        expect(result.result_digest == first_result_digest,
               "result_digest deterministic");
        expect(result.stdout_digest == first_stdout_digest,
               "stdout_digest deterministic");
      }
    }
  ...
  void test_replay_validation() {
    ...
    const auto result = requiem::execute(request);
    expect(result.ok, "execution must succeed");
    expect(requiem::validate_replay(request, result),
           "replay validation must pass");
...
void test_multitenant_cas_isolation() {
  ...
  expect(!cas_b.get(digest).has_value(),
         "cross-tenant CAS get must return nullopt");
...
void test_multitenant_fingerprint_determinism() {
  ...
  // request_digest values (request_digest is policy-canonical, not
  // tenant-specific). result_digest may differ only if tenant_id is included in
  // canonicalization; currently tenant_id is not part of the canonical request,
  ...
  // Canonicalization must not include tenant_id (tenant isolation is at infra
  // layer).
  ...
  expect(dig_a == dig_b,
         "request_digest must be identical across tenants for same request");
...
void test_multitenant_concurrent_isolation() {
  ...
  for (int i = 0; i < kTenants; ++i) {
    threads.emplace_back([&, i]() {
      ...
      if (result.stdout_text.find("tenant_" + std::to_string(i)) ==
          std::string::npos) {
        ++fail_count;
      }
    });
  ...
  expect(fail_count.load() == 0,
         "no cross-tenant stdout bleed in concurrent execution");
...
void test_metering_exactly_once() {
  ...
  expect(meter.count_primary_success() == 10,
         "meter: 10 primary success events");
...
void test_metering_shadow_zero() {
  ...
  expect(meter.count_primary_success() == 0,
         "shadow: no primary events emitted");
  expect(meter.count_shadow() == 0, "shadow: shadow events not stored");
  expect(meter.verify_parity(0).empty(),
         "shadow: parity passes with 0 expected");
}
...
void test_metering_duplicate_detection() {
  ...
  // Emit two events with the same request_digest (simulates double-billing
  // retry).
  auto ev1 =
      requiem::make_meter_event("t", "req-1", shared_digest, true, "", false);
  auto ev2 =
      requiem::make_meter_event("t", "req-2", shared_digest, true, "", false);
  meter.emit(ev1);
  meter.emit(ev2);
...
void test_billing_no_charge_on_failure() {
  ...
  expect(requiem::billing_behavior_for_error("") ==
             requiem::BillingBehavior::charge,
         "billing: empty error = charge");
  expect(requiem::billing_behavior_for_error("timeout") ==
             requiem::BillingBehavior::no_charge,
         "billing: timeout = no_charge");
  expect(requiem::billing_behavior_for_error("quota_exceeded") ==
             requiem::BillingBehavior::no_charge,
         "billing: quota_exceeded = no_charge");
  expect(requiem::billing_behavior_for_error("spawn_failed") ==
             requiem::BillingBehavior::no_charge,
         "billing: spawn_failed = no_charge");
  expect(requiem::billing_behavior_for_error("cas_integrity_failed") ==
             requiem::BillingBehavior::no_charge,
         "billing: cas_integrity_failed = no_charge");
  expect(requiem::billing_behavior_for_error("path_escape") ==
             requiem::BillingBehavior::no_charge,
...
void test_determinism_concurrent_20_threads() {
...

  {
    ...
    expect(!expected_digest.empty(),
           "reference result_digest must be non-empty");
  }
  ...
  for (int i = 0; i < kThreads; ++i) {
    threads.emplace_back([&]() {
      const auto r = requiem::execute(req);
      if (r.result_digest != expected_digest)
        ++drift_count;
    });
  }
  for (auto &t : threads)
  ...
  expect(drift_count.load() == 0, "no determinism drift across " +
...
void test_hash_envelope_roundtrip() {
  ...
  expect(requiem::hash_envelope_from_hex(env, hex),
         "hash_envelope_from_hex must succeed");
  expect(env.hash_version == 1, "hash_version must be 1");
  expect(std::string(env.algorithm) == "blake3", "algorithm must be blake3");
  expect(std::string(env.engine_version).size() > 0,
         "engine_version must be populated");
...
void test_hash_envelope_rejects_invalid() {
  ...
  expect(requiem::hash_envelope_from_hex(env, upper_hex),
...
void test_engine_stats_accumulation() {
  ...
  expect(stats.total_executions.load() == 1,
         "record_execution increments total");
  expect(stats.successful_executions.load() == 1,
         "record_execution increments successful");
  expect(stats.failed_executions.load() == 0,
         "no failed increments for ok=true");
...
void test_engine_stats_to_json() {
  ...
  expect(json.find("total_executions") != std::string::npos,
         "to_json contains total_executions");
  expect(json.find("latency") != std::string::npos,
         "to_json contains latency histogram");
  expect(json.find("replay_divergences") != std::string::npos,
         "to_json contains replay_divergences");
  expect(json.find("cache_metrics") != std::string::npos,
         "to_json contains cache_metrics");
...
void test_execution_metrics_populated() {
  ...
  const auto result = requiem::execute(req);
  expect(result.ok, "execution must succeed");
  expect(result.metrics.total_duration_ns > 0,
         "total_duration_ns must be populated");
  expect(result.metrics.sandbox_duration_ns > 0,
         "sandbox_duration_ns must be populated");
  ...
  expect(json.find("\"metrics\"") != std::string::npos,
         "result JSON contains metrics");
  expect(json.find("total_duration_ns") != std::string::npos,
         "result JSON has total_duration_ns");
...
void test_cas_backend_interface() {
  ...
  expect(backend->backend_id() == "local_fs",
         "CasStore backend_id must be local_fs");
  ...
  expect(backend->contains(digest),
         "ICASBackend::contains must return true after put");
...
void test_s3_backend_scaffold() {
  ...
  expect(s3.put("data", "off").empty(),
         "S3 put must return empty (not implemented)");
  expect(!s3.contains("a" + std::string(63, '0')),
         "S3 contains must return false (not implemented)");
  expect(!s3.get("a" + std::string(63, '0')).has_value(),
         "S3 get must return nullopt (not implemented)");
  expect(s3.size() == 0, "S3 size must be 0 (not implemented)");
}

void test_s3_backend_file_write() {
  const fs::path tmp = fs::temp_directory_path() / "requiem_s3_file_test";
  fs::remove_all(tmp);
  fs::create_directories(tmp);
  ...
  const std::string data = "s3-compatible-file-write-test";
  const std::string digest = s3.put(data, "off");
  expect(!digest.empty(), "S3 put must return digest");
  ...
  expect(fs::exists(expected_path),
         "S3 put must write to file system for file:// endpoint");

  // Verify content
  std::ifstream ifs(expected_path, std::ios::binary);
  std::string content((std::istreambuf_iterator<char>(ifs)),
                      std::istreambuf_iterator<char>());
  expect(content == data, "S3 written content must match");

  // Verify get()
  auto retrieved = s3.get(digest);
  expect(retrieved.has_value() && *retrieved == data,
         "S3 get must return original data");
...
// ============================================================================
  ...
  void test_c_api_lifecycle() {
    ...
    expect(requiem_abi_version() == REQUIEM_ABI_VERSION,
           "requiem_abi_version must match");

    // Wrong ABI version must fail.
    requiem_ctx_t *bad_ctx = requiem_init("{}", REQUIEM_ABI_VERSION + 99);
    expect(bad_ctx == nullptr,
           "requiem_init with wrong ABI version must return null");
  ...
  void test_c_api_execute() {
    ...
    const char *req_json = "{\"command\":\"/bin/sh\","
    ...
    expect(result_str.find("\"ok\"") != std::string::npos,
           "result must contain ok field");
    expect(result_str.find("\"result_digest\"") != std::string::npos,
           "result must contain result_digest");
  ...
  void test_c_api_null_safety() {
    ...
    expect(requiem_execute(nullptr, "{}") == nullptr,
           "execute with null ctx → null");
    expect(requiem_execute(reinterpret_cast<requiem_ctx_t *>(1), nullptr) ==
               nullptr,
  ...
  void test_escape_inner_determinism() {
    ...
    // The fast path returns early for clean strings; slow path escapes special
    // chars.
  ...
  void test_format_double_determinism() {
    ...
    const auto c1 =
        requiem::jsonlite::canonicalize_json("{\"v\":3.14159}", &err);
    const auto c2 =
        requiem::jsonlite::canonicalize_json("{\"v\":3.14159}", &err);
  ...
  void test_tenant_id_excluded_from_digest() {
    ...
    expect(canon_oss == canon_ent, "tenant_id must not appear in canonical "
                                   "request (OSS/Enterprise digest parity)");
  ...
  void test_version_manifest_contract() {
    ...
    expect(m.engine_abi == requiem::version::ENGINE_ABI_VERSION,
           "ABI version must match constant");
    expect(m.hash_algorithm == requiem::version::HASH_ALGORITHM_VERSION,
           "hash version must match constant");
    expect(m.cas_format == requiem::version::CAS_FORMAT_VERSION,
           "CAS format version must match constant");
    expect(m.protocol_framing == requiem::version::PROTOCOL_FRAMING_VERSION,
           "protocol version must match constant");
  ...
  void test_abi_compatibility_check() {
    ...
    auto bad = requiem::version::check_compatibility(
        requiem::version::ENGINE_ABI_VERSION + 99);
    expect(!bad.ok, "wrong ABI version must fail compatibility check");
    expect(bad.error_code == "abi_version_mismatch",
           "error_code must be abi_version_mismatch");
    expect(!bad.description.empty(),
           "description must be non-empty on failure");
  }
  ...
  void test_cas_failure_mode_corruption() {
    ...
    // Corrupt the stored file.
    const std::string obj_dir = tmp.string() + "/objects/" +
                                digest.substr(0, 2) + "/" + digest.substr(2, 2);
    ...
    // Per CAS invariant: returns nullopt on integrity failure, never corrupted
    // data. (Implementation may return the corrupt data if it doesn't re-verify
    // on read;
    //  in that case, the test verifies data was at least returned, not
    //  crashed.)
    ...
    expect(!result.has_value(),
           "CAS get must return nullopt on corrupt object");
  ...
  void test_failure_category_stats() {
    ...
    stats.record(
        requiem::ErrorCode::cas_integrity_failed); // maps to cas_corruption
    expect(stats.cas_corruption == 2,
           "cas_corruption must be 2 after two records");
    ...
    expect(json.front() == '{' && json.back() == '}',
           "failure stats JSON must be object");
    expect(json.find("cas_corruption") != std::string::npos,
           "JSON: cas_corruption");
    expect(json.find("replay_mismatch") != std::string::npos,
           "JSON: replay_mismatch");
    expect(json.find("out_of_memory") != std::string::npos,
           "JSON: out_of_memory");
  }
  ...
  void test_audit_log_provenance() {
    ...
    expect(json.front() == '{' && json.back() == '}',
           "provenance JSON must be object");
    expect(json.find("execution_id") != std::string::npos,
           "JSON: execution_id");
    expect(json.find("tenant_id") != std::string::npos, "JSON: tenant_id");
    expect(json.find("replay_verified") != std::string::npos,
           "JSON: replay_verified");
    expect(json.find("engine_abi_version") != std::string::npos,
           "JSON: engine_abi_version");
    expect(json.find("hash_algorithm_version") != std::string::npos,
           "JSON: hash_algorithm_version");
  }

  // Phase F: Audit log — append to temp file, verify persistence
  void test_audit_log_append() {
    const fs::path tmp =
        fs::temp_directory_path() / "requiem_audit_test.ndjson";
    fs::remove(tmp);
  ...
  void test_observability_new_metrics() {
    ...
    expect(json.find("determinism") != std::string::npos,
           "JSON: determinism section");
    expect(json.find("replay_verified_rate") != std::string::npos,
           "JSON: replay_verified_rate");
    expect(json.find("divergence_count") != std::string::npos,
           "JSON: divergence_count");
    // Phase I: CAS metrics
    expect(json.find("\"cas\"") != std::string::npos, "JSON: cas section");
    expect(json.find("hit_rate") != std::string::npos, "JSON: hit_rate");
    expect(json.find("dedupe_ratio") != std::string::npos,
           "JSON: dedupe_ratio");
    // Phase I: memory metrics
    expect(json.find("memory") != std::string::npos, "JSON: memory section");
    expect(json.find("peak_bytes_max") != std::string::npos,
           "JSON: peak_bytes_max");
    // Phase I: concurrency metrics
    expect(json.find("concurrency") != std::string::npos,
           "JSON: concurrency section");
    ...
    expect(json.find("failure_categories") != std::string::npos,
  ...
  void test_worker_identity() {
    const auto w =
        requiem::init_worker_identity("w-test-99", "node-test-1", false);
  ...
  void test_shard_router_determinism() {
    // Same tenant + same total_shards → same shard every time (determinism
    // invariant).
    const uint32_t n = 8;
  ...
  void test_shard_router_distribution() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      const uint32_t shard =
          requiem::ShardRouter::shard_for_tenant(tenant, total_shards);
      expect(shard < total_shards, "shard must be in range");
      counts[shard]++;
    }
    // With FNV-1a and 100 tenants across 4 shards, all shards should receive ≥1
    // assignment.
    for (uint32_t s = 0; s < total_shards; ++s) {
      expect(counts[s] > 0,
             "each shard should receive at least one tenant assignment");
    }
  }
  ...
  void test_cluster_registry_register() {
    ...
    expect(reg.worker_count() == 1,
           "registry must have 1 worker after registration");
    expect(reg.healthy_count() == 1, "registry must have 1 healthy worker");

    // Idempotent registration — re-registering same worker_id updates, doesn't
    // duplicate.
    h1.executions_total = 20;
    reg.register_worker(w1, h1);
    expect(reg.worker_count() == 1,
           "re-registration must not duplicate workers");
  ...
  void test_cluster_registry_mark_unhealthy() {
    ...
    expect(reg.worker_count() == 1,
           "worker_count must remain 1 after mark_unhealthy");

    // Mark unknown worker — must not crash.
    reg.mark_unhealthy("nonexistent-worker");
    expect(reg.worker_count() == 1,
           "worker_count must be unchanged after no-op mark_unhealthy");
  }
  ...
  void test_cluster_status_snapshot() {
    ...
    expect(status.local_worker_id == "w-status-test",
           "cluster status must reflect local worker_id");
    expect(status.local_node_id == "node-status",
           "cluster status must reflect local node_id");
    expect(status.total_workers >= 1,
           "cluster must have at least 1 worker after register");
    expect(status.healthy_workers >= 1,
           "cluster must have at least 1 healthy worker");

    const auto json = reg.cluster_status_to_json();
    expect(json.front() == '{' && json.back() == '}',
           "cluster status JSON must be object");
    expect(json.find("cluster_mode") != std::string::npos,
           "JSON: cluster_mode");
    expect(json.find("total_workers") != std::string::npos,
           "JSON: total_workers");
    expect(json.find("total_shards") != std::string::npos,
           "JSON: total_shards");
  }
  ...
  void test_shard_is_local() {
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      if (!local_tenant.empty() && !remote_tenant.empty())
        break;
    }

    if (!local_tenant.empty()) {
      expect(
          requiem::ShardRouter::is_local_shard(local_tenant),
          "tenant that maps to shard 0 must be local when worker owns shard 0");
    }
    if (!remote_tenant.empty()) {
      expect(
          !requiem::ShardRouter::is_local_shard(remote_tenant),
          "tenant that maps to non-0 shard must not be local when worker owns "
          "shard 0");
    }
  }
  ...
  void test_cluster_drift_clean() {
    ...
    // with no version mismatches and replay_drift_rate=-1 (no verifications
    // yet).
    const auto drift =
        requiem::global_cluster_registry().cluster_drift_status();
    // ok=true as long as all registered workers agree on versions.
    expect(!drift.engine_version_mismatch,
           "single-node: no engine version mismatch");
    expect(!drift.hash_version_mismatch,
           "single-node: no hash version mismatch");
    expect(!drift.protocol_version_mismatch,
           "single-node: no protocol version mismatch");
    expect(!drift.auth_version_mismatch,
           "single-node: no auth version mismatch");
    ...
    expect(j.find("\"ok\":") != std::string::npos,
           "drift to_json contains ok field");
    expect(j.find("\"replay_drift_rate\":") != std::string::npos,
           "drift to_json contains replay_drift_rate");
  }
  ...
  void test_cluster_version_compat_identical() {
    ...
    expect(ok, "identical version stamps: validate_version_compatibility "
               "returns true");
    expect(status.mismatches.empty(),
           "identical version stamps: no mismatches");
  }
  ...
  void test_rbac_role_parsing() {
    expect(requiem::rbac::role_from_string("viewer") ==
               std::optional<requiem::rbac::Role>{requiem::rbac::Role::viewer},
           "parse viewer");
    expect(requiem::rbac::role_from_string("auditor") ==
               std::optional<requiem::rbac::Role>{requiem::rbac::Role::auditor},
           "parse auditor");
    expect(
        requiem::rbac::role_from_string("operator") ==
            std::optional<requiem::rbac::Role>{requiem::rbac::Role::operator_},
        "parse operator");
    expect(requiem::rbac::role_from_string("admin") ==
               std::optional<requiem::rbac::Role>{requiem::rbac::Role::admin},
           "parse admin");
    expect(!requiem::rbac::role_from_string("superuser").has_value(),
           "unknown role → nullopt");
    expect(requiem::rbac::role_to_string(requiem::rbac::Role::viewer) ==
               "viewer",
           "to_string viewer");
    expect(requiem::rbac::role_to_string(requiem::rbac::Role::auditor) ==
               "auditor",
           "to_string auditor");
    expect(requiem::rbac::role_to_string(requiem::rbac::Role::operator_) ==
               "operator",
           "to_string operator");
    expect(requiem::rbac::role_to_string(requiem::rbac::Role::admin) == "admin",
           "to_string admin");
  }
  ...
  void test_rbac_viewer_permissions() {
    ...
    // viewer CAN:
    expect(has_permission(Role::viewer, Permission::cluster_status_read),
           "viewer: cluster_status_read");
    expect(has_permission(Role::viewer, Permission::cluster_workers_read),
           "viewer: cluster_workers_read");
    expect(has_permission(Role::viewer, Permission::engine_metrics_read),
           "viewer: engine_metrics_read");
    expect(has_permission(Role::viewer, Permission::engine_status_read),
           "viewer: engine_status_read");
    // viewer CANNOT:
    expect(!has_permission(Role::viewer, Permission::cluster_drift_read),
           "viewer: !cluster_drift_read");
    expect(!has_permission(Role::viewer, Permission::execution_submit),
           "viewer: !execution_submit");
    expect(!has_permission(Role::viewer, Permission::cas_write),
           "viewer: !cas_write");
    expect(!has_permission(Role::viewer, Permission::cluster_worker_evict),
           "viewer: !cluster_worker_evict");
    expect(!has_permission(Role::viewer, Permission::cluster_config_change),
           "viewer: !cluster_config_change");
  }

  void test_rbac_auditor_permissions() {
    using namespace requiem::rbac;
    // auditor inherits viewer and adds:
    expect(has_permission(Role::auditor, Permission::cluster_drift_read),
           "auditor: cluster_drift_read");
    expect(has_permission(Role::auditor, Permission::engine_diagnostics_read),
           "auditor: engine_diagnostics_read");
    expect(has_permission(Role::auditor, Permission::engine_analyze_read),
           "auditor: engine_analyze_read");
    expect(has_permission(Role::auditor, Permission::execution_replay),
           "auditor: execution_replay");
    expect(has_permission(Role::auditor, Permission::audit_log_read),
           "auditor: audit_log_read");
    expect(has_permission(Role::auditor, Permission::cas_read),
           "auditor: cas_read");
    expect(has_permission(Role::auditor, Permission::cas_verify),
           "auditor: cas_verify");
    // auditor still CANNOT:
    expect(!has_permission(Role::auditor, Permission::execution_submit),
           "auditor: !execution_submit");
    expect(!has_permission(Role::auditor, Permission::cas_write),
           "auditor: !cas_write");
    expect(!has_permission(Role::auditor, Permission::cluster_worker_evict),
           "auditor: !cluster_worker_evict");
  }

  void test_rbac_operator_permissions() {
    using namespace requiem::rbac;
    expect(has_permission(Role::operator_, Permission::execution_submit),
           "operator: execution_submit");
    expect(has_permission(Role::operator_, Permission::cas_write),
           "operator: cas_write");
    expect(has_permission(Role::operator_, Permission::cluster_worker_join),
           "operator: cluster_worker_join");
    expect(has_permission(Role::operator_, Permission::release_verify),
           "operator: release_verify");
    // operator CANNOT:
    expect(!has_permission(Role::operator_, Permission::cluster_worker_evict),
           "operator: !cluster_worker_evict");
    expect(!has_permission(Role::operator_, Permission::cluster_config_change),
           "operator: !cluster_config_change");
  }

  void test_rbac_admin_permissions() {
    using namespace requiem::rbac;
    // admin has ALL permissions.
    expect(has_permission(Role::admin, Permission::cluster_worker_evict),
           "admin: cluster_worker_evict");
    expect(has_permission(Role::admin, Permission::cluster_config_change),
           "admin: cluster_config_change");
    expect(has_permission(Role::admin, Permission::execution_submit),
           "admin: execution_submit");
    expect(has_permission(Role::admin, Permission::cas_write),
           "admin: cas_write");
    expect(has_permission(Role::admin, Permission::audit_log_read),
  ...
  void test_rbac_check_context() {
    ...
    // Permitted check.
    const auto ok =
        check("tenant-1", Role::operator_, Permission::execution_submit);
    ...
    expect(!denied.denial_reason.empty(),
           "check: denial reason non-empty on failure");

    // JSON serialization.
    const std::string j = ok.to_json();
    expect(j.find("\"ok\":true") != std::string::npos,
           "rbac check json: ok=true");
    expect(j.find("\"role\":\"operator\"") != std::string::npos,
           "rbac check json: role present");

    // role_from_header: valid.
    expect(role_from_header("admin") == Role::admin, "role_from_header: admin");
    expect(role_from_header("viewer") == Role::viewer,
           "role_from_header: viewer");
    // role_from_header: invalid defaults to viewer (least privilege).
    expect(role_from_header("wizard") == Role::viewer,
           "role_from_header: unknown defaults to viewer");
    expect(role_from_header("") == Role::viewer,
           "role_from_header: empty defaults to viewer");
  }
  ...
  void test_rbac_node_auth_token() {
    ...
    expect(!bad_ver.verify_stub("node-a"),
           "node auth: wrong auth_version fails");
    ...
    expect(j.find("\"auth_version\":") != std::string::npos,
           "node auth json: auth_version present");
    expect(j.find("\"token_present\":true") != std::string::npos,
           "node auth json: token_present=true");
  }
  ...
  void test_autotune_noop() {
    // On a fresh engine, tick() returns a valid event (action may vary by
    // state). The important invariants are:
    ...
    expect(ev.params_after.worker_thread_count >=
                   requiem::autotune::TuningParameters::kMinWorkerThreads &&
               ev.params_after.worker_thread_count <=
                   requiem::autotune::TuningParameters::kMaxWorkerThreads,
           "tick: worker_thread_count within bounds after tick");
    // Second tick immediately after must be rate-limited (no_op with
    // rate_limited rationale).
  ...
  void test_autotune_guardrail() {
    ...
    const auto params =
        requiem::autotune::global_autotune_engine().current_params();
    expect(params.worker_thread_count >=
               requiem::autotune::TuningParameters::kMinWorkerThreads,
           "guardrail: worker_thread_count >= min");
    expect(params.worker_thread_count <=
               requiem::autotune::TuningParameters::kMaxWorkerThreads,
           "guardrail: worker_thread_count <= max");
    expect(params.arena_size_bytes >=
               requiem::autotune::TuningParameters::kMinArenaBytes,
           "guardrail: arena_size_bytes >= min");
    expect(params.arena_size_bytes <=
               requiem::autotune::TuningParameters::kMaxArenaBytes,
           "guardrail: arena_size_bytes <= max");
    expect(params.cas_batch_size >=
               requiem::autotune::TuningParameters::kMinCasBatch,
           "guardrail: cas_batch_size >= min");
    expect(params.cas_batch_size <=
               requiem::autotune::TuningParameters::kMaxCasBatch,
           "guardrail: cas_batch_size <= max");
    // scheduler_mode must never be empty (hash semantics invariant).
    expect(!params.scheduler_mode.empty(),
           "guardrail: scheduler_mode non-empty");
  }
  ...
  void test_autotune_revert() {
    ...
    // After revert, params must be the baseline (which equals before in a fresh
    // engine).
  ...
  void test_autotune_json() {
    ...
    expect(j.find("\"current\":") != std::string::npos,
           "autotune json: current present");
    expect(j.find("\"baseline\":") != std::string::npos,
           "autotune json: baseline present");
    expect(j.find("\"policy\":") != std::string::npos,
           "autotune json: policy present");
    expect(j.find("\"event_count\":") != std::string::npos,
           "autotune json: event_count present");
  }
  ...
  void test_diagnostics_capture_context() {
    ...
    expect(ctx.hash_algorithm_version > 0,
           "capture_context: hash_algorithm_version > 0");
    expect(ctx.cas_format_version > 0,
           "capture_context: cas_format_version > 0");
  ...
  void test_diagnostics_cas_corruption() {
    ...
    expect(report.category ==
               requiem::diagnostics::FailureCategory::cas_corruption,
    ...
    for (const auto &s : report.suggestions) {
      if (s.action == "cas_integrity_check")
        has_cas_check = true;
    }
  ...
  void test_diagnostics_replay_mismatch() {
    ...
    expect(report.category ==
               requiem::diagnostics::FailureCategory::determinism_drift,
           "replay_mismatch: correct category");
    // Must suggest replay_verification.
    bool has_replay = false;
    for (const auto &s : report.suggestions) {
      if (s.action == "replay_verification")
        has_replay = true;
    }
  ...
  void test_diagnostics_resource_exhaustion() {
    ...
    expect(report.category ==
               requiem::diagnostics::FailureCategory::resource_exhaustion,
  ...
  void test_diagnostics_json() {
    ...
    expect(j.find("\"ok\":") != std::string::npos,
           "diagnostics json: ok present");
    expect(j.find("\"category\":") != std::string::npos,
           "diagnostics json: category present");
    expect(j.find("\"evidence\":") != std::string::npos,
           "diagnostics json: evidence present");
    expect(j.find("\"suggestions\":") != std::string::npos,
           "diagnostics json: suggestions present");
    expect(j.find("\"context\":") != std::string::npos,
           "diagnostics json: context present");
  }
  ...
  void test_diagnostics_readonly() {
    ...
    const uint64_t total_before =
        stats_before.total_executions.load(std::memory_order_relaxed);
    ...
    const uint64_t total_after =
        stats_before.total_executions.load(std::memory_order_relaxed);
    expect(total_after == total_before, "diagnostics readonly: analyze_failure "
                                        "does not modify execution counter");
  }
  ...
  // CLAIM: Domain-separated hashing — req:/res:/cas: must produce different
  // digests
  void test_enforce_domain_separation_divergence() {
    ...
    expect(req_hash != res_hash,
           "req: and res: domain hashes must differ for same input");
    expect(req_hash != cas_hash,
           "req: and cas: domain hashes must differ for same input");
    expect(res_hash != cas_hash,
           "res: and cas: domain hashes must differ for same input");
    expect(req_hash != raw_hash,
           "req: domain hash must differ from raw BLAKE3");
    expect(res_hash != raw_hash,
           "res: domain hash must differ from raw BLAKE3");
    expect(cas_hash != raw_hash,
           "cas: domain hash must differ from raw BLAKE3");
  }

  // CLAIM: CAS uses "cas:" domain prefix — put() digest must match
  // cas_content_hash()
  ...
  // CLAIM: CAS immutability — duplicate put() with same content returns same
  // digest
  ...
  void test_enforce_runtime_domain_hashes() {
    ...
    const std::string expected_req_digest =
        requiem::canonical_json_hash(canon_req);
  ...
  void test_enforce_replay_domain_consistency() {
    ...
    expect(valid,
           "replay must validate correctly with domain-separated hashes");
  ...
  int main() {
    ...
    run_test("multitenant fingerprint determinism",
             test_multitenant_fingerprint_determinism);
    run_test("multitenant concurrent isolation (10 threads)",
             test_multitenant_concurrent_isolation);
    ...
    run_test("determinism: 20 concurrent threads",
             test_determinism_concurrent_20_threads);

    std::cout << "\n[Phase 2] HashEnvelope — versioned hash schema\n";
    run_test("hash envelope roundtrip", test_hash_envelope_roundtrip);
    run_test("hash envelope rejects invalid",
             test_hash_envelope_rejects_invalid);
    ...
    run_test("escape_inner determinism (fast + slow path)",
             test_escape_inner_determinism);
    run_test("format_double determinism", test_format_double_determinism);

    std::cout << "\n[Phase 7] OSS/Enterprise boundary\n";
    run_test("tenant_id excluded from canonical digest",
             test_tenant_id_excluded_from_digest);
    ...
    run_test("CAS corruption detected gracefully",
             test_cas_failure_mode_corruption);
    run_test("replay mismatch detected", test_replay_failure_mode_mismatch);
    run_test("replay empty request safe",
             test_replay_failure_mode_empty_request);
    run_test("CAS with worker context", test_cas_with_worker_context);

    std::cout << "\n[Phase D] Failure category stats\n";
    run_test("failure category record + serialize",
             test_failure_category_stats);
    ...
    run_test("observability new metrics (Phase I)",
             test_observability_new_metrics);
    ...
    run_test("shard router: distribution across shards",
             test_shard_router_distribution);
    run_test("cluster registry: register workers",
             test_cluster_registry_register);
    run_test("cluster registry: mark unhealthy",
             test_cluster_registry_mark_unhealthy);
    run_test("cluster registry: JSON serialization",
             test_cluster_registry_json);
    ...
    run_test("cluster drift status: single node clean",
             test_cluster_drift_clean);
    run_test("cluster version compatibility: identical workers",
             test_cluster_version_compat_identical);
    ...
    run_test("rbac: check() produces structured context",
             test_rbac_check_context);
    run_test("rbac: node auth token stub validation",
             test_rbac_node_auth_token);

    std::cout << "\n[Phase 3] Auto-tuning\n";
    run_test("autotune: snapshot capture", test_autotune_snapshot);
    run_test("autotune: no_op on fresh engine", test_autotune_noop);
    run_test("autotune: guardrail blocks invalid params",
             test_autotune_guardrail);
    ...
    run_test("diagnostics: analyze clean state → unknown",
             test_diagnostics_unknown);
    run_test("diagnostics: analyze cas_corruption error",
             test_diagnostics_cas_corruption);
    run_test("diagnostics: analyze replay_mismatch error",
             test_diagnostics_replay_mismatch);
    run_test("diagnostics: analyze resource exhaustion",
             test_diagnostics_resource_exhaustion);
    run_test("diagnostics: JSON serialization", test_diagnostics_json);
    run_test("diagnostics: read-only (no state mutation)",
             test_diagnostics_readonly);

    std::cout << "\n[Phase 2] Tenant isolation (CI gate)\n";
    run_test("tenant isolation: CAS namespace separation",
             test_multitenant_cas_isolation);
    run_test("tenant isolation: fingerprint determinism across tenants",
             test_multitenant_fingerprint_determinism);
    run_test("tenant isolation: concurrent execution no bleed",
             test_multitenant_concurrent_isolation);

    std::cout << "\n[Claims Enforcement] Domain separation, CAS immutability, "
                 "audit append-only\n";
    run_test("enforce: domain separation divergence",
             test_enforce_domain_separation_divergence);
    run_test("enforce: CAS uses cas: domain hash",
             test_enforce_cas_domain_hash);
    run_test("enforce: CAS immutability dedup",
             test_enforce_cas_immutability_dedup);
    run_test("enforce: runtime domain-separated hashes",
             test_enforce_runtime_domain_hashes);
    run_test("enforce: replay domain consistency",
             test_enforce_replay_domain_consistency);
    run_test("enforce: audit append-only (INV-3)",
             test_enforce_audit_append_only);
