...
void test_cas_failure_mode_corruption() {
  ...
  // Corrupt the stored file.
  const std::string obj_dir = tmp.string() + "/objects/" + digest.substr(0, 2) +
                              "/" + digest.substr(2, 2);
...
void test_cluster_version_compat_identical() {
  ...
  expect(
      ok,
      "identical version stamps: validate_version_compatibility returns true");
...
void test_rbac_role_parsing() {
  ...
  expect(requiem::rbac::role_from_string("auditor") ==
         ...
         "parse auditor");
  expect(requiem::rbac::role_from_string("operator") ==
  ...
  expect(requiem::rbac::role_to_string(requiem::rbac::Role::viewer) == "viewer",
         "to_string viewer");
...
void test_autotune_noop() {
  ...
  // Second tick immediately after must be rate-limited (no_op with rate_limited
  // rationale).
